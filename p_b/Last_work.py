...
class Sear(tk.Toplevel):
    ...
    def init_search(self):
        ...
        label_sear = tk.Label(self, text="Имя:")
        label_sear.place(x=50, y=20)

        self.entry_sear = ttk.Entry(self)
        self.entry_sear.place(x=100, y=20, width=150)

        btn_cancel = ttk.Button(self, text="Закрыть", command=self.destroy)
        btn_cancel.place(x=185, y=50)

        sear_btn = ttk.Button(self, text="Найти")
        sear_btn.place(x=105, y=50)
        sear_btn.bind(
            "<Button-1>",
            lambda event: self.view.search_records(self.entry_sear.get()),
        )
        sear_btn.bind("<Button-1>", lambda event: self.destroy(), add="+")
...
class DB:
    ...
    def insert_data(self, name, tell, email):
        self.cursor.execute(
            """INSERT INTO db(name, tel, email) VALUES(?, ?, ?)""", (name, tell, email)
        )
        self.conn.commit()
